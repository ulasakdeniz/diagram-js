...
ReplaceShapeHandler.prototype.execute = function(context) {
  ...
  if (hints.moveChildren !== false) {
    var children = newShape.children.slice();

    // hide/show children, if replacing collapsed, expanded shape
    if (oldShape.collapsed !== newShape.collapsed) {
      var visible = newShape.collapsed;
...
ReplaceShapeHandler.prototype.revert = function(context) {
  ...
  if (hints.moveChildren !== false) {
    ...
    // undo hiding/showing of children
    if (oldShape.collapsed !== newShape.collapsed) {
      var visible = oldShape.collapsed;
      children.forEach(function(child) {
        child.hidden = visible;
      });
    }
